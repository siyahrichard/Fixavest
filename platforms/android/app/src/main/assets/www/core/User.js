...
User.register=function(user)
{
	...
	{
		var phone=_("#phoneTxb").value();
		var country=_("#countryCode").value();
		phone=parseInt(phone.replace(/\-/g,''));
		country=parseInt(country.replace(/\-/g,''));
		
	    User.activeObject=new User(0,_("#userTxb").value(),_("#emailTxb").value(),phone,_("#pass1Txb").value());
	    User.activeObject.country=country;
	    user=new User();
...
User.login=function(loginName,password,country)
{
	...
	if(error===0)
	{
	    ...
	    n.clear();
	    if(HomeCore.phoneMode){
	    	var phone=_("#phoneTxbL").value();
			var country=_("#countryCodeL").value();
			phone=parseInt(phone.replace(/\-/g,''));
			country=parseInt(country.replace(/\-/g,''));
			
	    	n.add("country",country);
	    	n.add("loginName",phone);
	    }else
...
{
	if(typeof(jsClientApp)=="undefined"){
		if(res=="ok"){
		...
		//js app
		if(res=='error')alert("unable to login. (maybe username or password is wrong )");
		else{
			if(localStorage){
				//result in cert,hub,country format
				var parts=res.split(',');
				localStorage.setItem('user_cert',parts[0]);
				localStorage.setItem('user_hub',parts[1]);
				localStorage.setItem('country',parts[2]);
				localStorage.setItem('user_fp',User.fingerPrint);
				window.location.href=User.panelPath;
			}
		}
	}
};
